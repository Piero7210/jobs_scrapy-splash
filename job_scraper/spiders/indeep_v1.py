...
class IndeepV1Spider(scrapy.Spider):
    name = "indeep_v1"
    allowed_domains = ["indeed.com", "localhost"]
    start_urls = []

    # Diccionario para rastrear el número de trabajos obtenidos por palabra clave
    job_counts = {}

    # Generar las URLs iniciales para cada palabra clave
    def __init__(self, *args, **kwargs):
        super(IndeepV1Spider, self).__init__(*args, **kwargs)
        ...
        for keyword in keywords_jobs:
            keyword_lower = keyword.lower()  # Convertimos la palabra clave a minúsculas
            self.job_counts[keyword_lower] = 0  # Inicializa el contador para esta palabra clave
            self.start_urls.extend(
                [f'https://pe.indeed.com/jobs?q={keyword}&l=Lima&sort=date&fromage=7&start={i}' for i in
                 range(0, 70, 10)])
    ...
    def parse(self, response):
        ...
        # Determinar la palabra clave de la URL (normalizada a minúsculas)
        keyword = response.url.split('q=')[-1].split('&')[0].lower()

        # Si ya hemos procesado 20 trabajos para esta palabra clave, detén el scraping
        if self.job_counts[keyword] >= 20:
            self.log(f"Se han obtenido 20 trabajos para la palabra clave '{keyword}', deteniendo búsqueda.")
            return
        ...
        for job in jobs:
            if self.job_counts[keyword] >= 20:
                break  # Detenemos el bucle si ya hemos alcanzado el límite de trabajos
            ...
            try:
                ...
                item['keyword'] = response.url.split('q=')[-1].split('&')[0].lower()  # Palabra clave utilizada para la búsqueda
                ...
                # Incrementamos el contador de trabajos para esta palabra clave
                self.job_counts[keyword] += 1
        ...
        try:
            ...
            if target_div:
                for element in response.css(
                        'div.jobsearch-JobComponent-description p, div.jobsearch-JobComponent-description li'):
            ...
            if type_of_job in ['Tiempo completo', 'Tiempo parcial', 'Por contrato', 'Indefinido', 'Temporal',
                               'Medio tiempo']:
            ...
            item['seniority'] = str(
                result_keywords.get('Seniority', 'no especificado')) if result_keywords else 'no especificado'
            item['work_mode'] = str(
                result_keywords.get('Modalidad de trabajo', 'Presencial')) if result_keywords else 'Presencial'
