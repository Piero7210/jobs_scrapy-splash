...
class ComputrabajoV1Spider(scrapy.Spider):
    name = "computrabajo_v1"
    ...
    def __init__(self, *args, **kwargs):
        super(ComputrabajoV1Spider, self).__init__(*args, **kwargs)
        ...
        for keyword in keywords_jobs:
            keyword_lower = keyword.lower()  # Convertir la palabra clave a minúsculas
            self.job_counts[keyword_lower] = 0  # Inicializa el contador de trabajos para cada palabra clave
            self.start_urls.extend(
                [f'https://pe.computrabajo.com/trabajo-de-{keyword}?by=publicationtime&pubdate=7&p={i}' for i in
                 range(1, 6)])
    ...
    def parse(self, response):
        ...
        # Obtén la palabra clave desde la URL para llevar el conteo
        keyword = response.url.split('/')[-1].split('-')[-1].split('?')[0]
        ...
        # Si ya hemos alcanzado 20 trabajos para esta palabra clave, detén el scraping
        ...
        for job in range(min(len(company_elements), len(title_elements), len(location_elements), len(date_elements))):
            ...
            if self.job_counts[keyword] >= 20:
                break  # Si llegamos a 20 trabajos, detenemos el bucle
            ...
            try:
                ...
                # Incrementamos el contador para esta palabra clave
    ...
    def parse_job_description(self, response):
        ...
        try:
            ...
            job_description = response.xpath(
                "//div[@class='mb40 pb40 bb1'][@div-link='oferta']//p[@class='mbB']/text()").getall()
            job_description = "\n".join(job_description).strip() if job_description else ''
            item['description'] = job_description

            # Obtiene el tipo de empleo
            type_of_job = response.xpath(
                "//div[@class='mb40 pb40 bb1'][@div-link='oferta']//div[@class='mbB']//span[@class='tag base mb10'][3]/text()").get()
