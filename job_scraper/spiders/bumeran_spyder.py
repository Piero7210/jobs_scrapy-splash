...
class BumeranSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        Returns:
            None
        """
        # Inspecciona el HTML recibido
        self.log(response.text)  # Esto imprimirá los primeros 1000 caracteres del HTML en el log
        
        # Obtiene los elementos de la página
        company_elements = response.css('h3.sc-dCVVYJ.kpFBFp')
        ...
        for job in range(min(len(company_elements), len(title_elements), len(location_elements), len(date_elements))):
            ...
            try:
                ...
                job_description_href = response.css('a.sc-kJdAmE.etbjmW::attr(href)').getall()[job]
